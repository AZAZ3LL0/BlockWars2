Shader "Custom/DeepShader"
{
    Properties
    {
        ...
        _SunPower("SunPower", Range(0, 1)) = 0.5
        _MinSunFade("MinSunFade", Range(0, 1)) = 0.4
        _MaxSunFade("MaxSunFade", Range(0, 1)) = 0.5
    ...
    SubShader
    {
        ...
        float3 getGrad(float x, float min_x, float max_x) {
            if (x > min_x && x < max_x) {
                float3 total_color = - (x - min_x) * (x - max_x);
                total_color = total_color / ((min_x + max_x) * (min_x + max_x) / 4 - min_x * max_x);
                return total_color;
            }
            return 0;
        }
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            o.Albedo = c.rgb * (getGrad(color_out, _MinSunFade, _MaxSunFade) * sun_power + 1);
